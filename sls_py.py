...
REMOTE_IP_ADDR		= "192.168.5.40"
UDP_PORT 	= 21234

LOCAL_TEST = 0

# Define [IP:Port] of Gateway
if LOCAL_TEST==1:
	TCP_IP = LOCAL_IP_ADDR
else:
	TCP_IP = REMOTE_IP_ADDR
...
for num in range(1,max_num+1):
	#make frame
	SFD 	= 0x7F
	len 	= 5			# node ID or IDs of multicast group
	seq1 	= 0x00
	seq0 	= num
	typ 	= MSG_TYPE_REQ
	cmd 	= CMD_GW_MULTICAST_CMD				#CMD_RF_LED_OFF    #CMD_GW_BROADCAST_CMD #CMD_GW_MULTICAST_CMD
	#cmd = 0xF6 + (num % 2)
	err 	= 0x00
	...
	if (num % 2)==0:
		multicast_cmd = CMD_RF_LED_OFF 			#CMD_RF_LED_DIM
	else:		
		multicast_cmd = CMD_RF_LED_ON 			#CMD_RF_LED_DIM

	multicast_val1 = 2 		# value of multicast_cmd
	multicast_val2 = 0
	multicast_val3 = 0
	multicast_val4 = 0
	multicast_val5 = 0
	...
	#concat 32 bytes to a command message
	MESSAGE = bytearray([SFD,len,seq1, seq0,typ, cmd, err, \
		multicast_cmd, \
		...
		1,2,3,4,5,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00])
	...
	print "len=", hex(MESSAGE[1]),"; seq=",hex(MESSAGE[2]),hex(MESSAGE[3]),"; type=",hex(MESSAGE[4]),"; cmd=",hex(MESSAGE[5]),"; err=",hex(MESSAGE[6])
	print "data=[",
	for i in range(7,MAX_CMD_LEN):
		print MESSAGE[i],
	print "]"
	s.send(MESSAGE)
	...
	reply = map(ord, data)
	print "len=", hex(reply[1]),"; seq=", hex(reply[2]), hex(reply[3]),"; type=", hex(reply[4]), "; cmd=", hex(reply[5]),"; err=",hex(reply[6])
	print "data=[",
	for i in range(7,MAX_CMD_LEN):
		print reply[i],
	print "]"
	...
	num_of_req_sent = num_of_req_sent + int(hex(reply[7]),0)
	num_of_ack_rev  = num_of_ack_rev  + int(hex(reply[8]),0)
	num_of_timout   = num_of_timout   + int(hex(reply[9]),0)

if (cmd==CMD_GW_BROADCAST_CMD) or (cmd == CMD_GW_MULTICAST_CMD):
	print ""
	print "-------------" + PINK + "SUMMARY BROADCAST/MULTICAST GW command" +RESET+"------------"
	print "Num of requests sent/received/time_out = " + GREEN ,num_of_req_sent,RESET + "/" + GREEN ,num_of_ack_rev, RESET + "/" + GREEN, num_of_timout, RESET + ""
	print "Success rate (%) = " + GREEN, float(num_of_ack_rev)*100/float(num_of_req_sent), RESET + ""
	print "---------------------------------------------------------------"

print ""
print "-------------" + PINK + "SUMMARY OF CLIENT command" + RESET + "-------------------------"
print "Num of iteration = " + GREEN, num, RESET + ""
print "Num of commands sent/replies received = " + GREEN, num_of_pkt_sent, "/",num_of_pkt_rev, RESET + ""
print "Success rate (%) = " + GREEN, float(num_of_pkt_rev)*100/float(num_of_pkt_sent), RESET + ""
print "Average delay/command (ms) = ", total_delay/num_of_pkt_sent
print "---------------------------------------------------------------"